"""
Sightline — Simulated Annealing Pathfinder
============================================
"""
...
class SABrain(Brain):
    ...
    # ------------------------------------------------------------------
    # Cost model — how "expensive" a path is (hazards, traps, Joker
    # proximity all raise cost; the annealer is trying to minimize this).
    # ------------------------------------------------------------------
    ...
    def _path_cost(self, start, path, goal, grid, joker_pos, joker_alive):
        ...
        for step in path:
            ...
            cur = step
        cost += manhattan(cur, goal) * 3  # heavy penalty for not actually arriving
        return cost
    ...
    def _greedy_random_path(self, start, goal, grid, greedy_bias=0.75):
        """A rough starting guess: mostly steps toward the goal, sometimes
        a random step, capped in length so it never runs away."""
        path = []
        cur = start
        visited = {start}
        for _ in range(self.sa_max_path_len):
            ...
            if not options:
                break
            options.sort(key=lambda p: manhattan(p, goal))
    ...
    # ------------------------------------------------------------------
    # THE ANNEALING LOOP — this is the actual "SA" part of the algorithm
    # ------------------------------------------------------------------
    def simulated_annealing_search(self, start, goal, grid, joker_pos=None, joker_alive=True):
        ...
        for _ in range(self.sa_iterations):
            ...
            # Propose a neighbor solution: either a fresh greedy-random
            # walk, or a mutation (truncate-and-regrow) of the current one.
